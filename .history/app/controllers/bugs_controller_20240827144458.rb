class BugsController < ApplicationController
  ...
  if current_user.developer?
    if @bug.update(bug_params_for_developer)
      redirect_to project_bug_path(@project, @bug), notice: 'Bug status was successfully updated.'
    else
      render :edit
    end
  else
  ...
  private
    ...
    def bug_params_for_developer
  params.require(:bug).permit(:status)
    end

    # Only allow a list of trusted parameters through.
    def bug_params
  params.require(:bug).permit(:description, :status, :deadline, :bug_type, :screenshot)
end
    def set_project
    @project = Project.find(params[:project_id])
  end
