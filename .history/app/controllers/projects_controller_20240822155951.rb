...
#   def search
#   @projects = if params[:name_search].present?
#                 Project.where("name LIKE ?", "%#{params[:name_search]}%")
#               else
#                 Project.none
#               end
...
#   #   #   render turbo_stream: turbo_stream.update("search_results", partial: "projects/search_results", locals: { projects: @projects })
#   #   # end
#   #   # format.html { render partial: "projects/search_results", locals: { projects: @projects } } # Optional: Handle HTML format if needed
#   #   format.json { render json: @projects } # Optional: Handle JSON format if needed
#   # end
# end
  ...
  # end
   def search
    if params[:name_search].present?
      @projects = Project.where("name LIKE ?", "%#{params[:name_search]}%")
    else
      @projects = Project.all
    end

      turbo_stream: turbo_stream.update("search_results", partial: "projects/search_results", locals: { projects: @projects })
